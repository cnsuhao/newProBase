...
bool LayerInstanceView::init() 
{
	...
	CHECKF(this->initButtonListen("ImageViewStrategy", [=](Ref*){
		...
		CHECK_NOLOG(sence);
		auto layer = LayerInstanceStrategy::create();
		CHECK_NOLOG(layer);
...
// 初始化监听按钮
bool LayerInstanceView::initButtonListen(const std::string& strName, const ui::Widget::ccWidgetClickCallback& callback)
{
	CHECKF(m_pCsbWidget);
